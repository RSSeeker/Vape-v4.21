...
extends Mod {
    ...
    public ModuleDisplayInfo getModuleDisplayInfo() {
        try {
            ...
            if (player.isNull()) {
                Vape.debugLog("[Health] getModuleDisplayInfo: player is null");
                return null;
            }
            double healthHearts = (double)player.w$src$F$15l9epb() / 2.0;
            Vape.debugLog("[Health] getModuleDisplayInfo: health=" + player.w$src$F$15l9epb()
                    + " absorption=" + player.p() + " hearts=" + healthHearts
                    + " enabled=" + this.isEnabled() + " activeModules="
                    + ClientSettings.getFrame(ActiveModuleStackFrame.class).V$src$Z$1xhop3l());
            if (player.p() > 0.0f) {
                heartColorCode = "§6";
            ...
            Vape.debugLog("[Health] displayText=" + displayText);
            return new ModuleDisplayInfo(displayText, displayColor, healthText, null);
        }
        catch (Throwable throwable) {
            Vape.logThrowable(throwable);
            Vape.debugLog("[Health] getModuleDisplayInfo threw: " + throwable);
            return null;
        }
    }
