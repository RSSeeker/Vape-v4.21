...
- **性能修复**：移除渲染线程每帧高频日志写入，解决「显示 100+ FPS 但肉眼约 30 FPS」的卡顿
- **移除注入超时降级**：帧初始化完成前不再虚假报告「注入完成」；GL 未就绪时注入保持「进行中」，避免误判
- **26.2 图形后端修复**：26.2 引入 Vulkan 后端，Vape 基于 OpenGL，需将图形 API 切换为 OpenGL 后使用（详见下方兼容性说明）
...
- 部分映射任务（鼠标点击事件、实体加入事件、3D 渲染事件、网络包事件、计分板渲染）在
  1.20.1 / 1.21.1 上可能提示「注入出错」，仅影响对应功能，其余功能不受影响
- 部分 HUD 覆盖层（如生命值覆盖层）定位可能异常（渲染位置偏移，如出现在右下角）
- 平滑字体初始化失败时会回退 legacy 字体渲染，偶发 GUI 边缘出现黑边
- 若遇崩溃请反馈日志

**26.1.2 为实验性适配，可能存在以下问题**：

- **部分 GUI 页 / 药水图标不显示**：用到 `TextureAtlasSprite` 坐标的渲染（如点击 GUI 某一页显示的
  药水效果图标）在 1.20.6+ 结构下读不到 `u0/u1/v0/v1` 字段而 NPE，导致该页 GUI 不渲染
- 部分渲染元素位置可能略有偏差（实验性）
- 若遇崩溃请反馈日志

**1.21.11 可能存在以下问题**：

- **Tracers 射线中心偏移**：世界空间的射线起点相对准星存在偏移（仅在面对 -Z 时对准）。原因：射线起点用
  `getPlayerView` 计算的视线方向，与该版本渲染事件的实际相机/投影矩阵不完全一致，存在随视角变化的偏移。
  **仅为外观问题，功能正常**，不影响其他版本
- 若遇崩溃请反馈日志
