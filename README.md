...
- **ItemESP 物品名字标签文本偏移**：世界空间标注的物品名文字相对背景框存在偏移（背景框位置正确，文字偏移，
  随视角/距离/缩放变化）。原因：背景框走世界投影的 shader 批量，文字走 MC 字体回调，两者投影不一致
  （26.1.2 的 MC 字体渲染为游戏内部实现，暂无法可靠统一）。**仅为外观问题，功能正常**，不影响其他版本
- 若遇崩溃请反馈日志
